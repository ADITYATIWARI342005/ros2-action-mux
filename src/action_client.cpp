...
void ActionClient::send_goal(const string & data)  
{  
  lock_guard<mutex> lock(client_mutex_);  
  ...
  // Create and populate a new goal  
  ...
  // Setup options for sending the goal  
  auto send_goal_options = rclcpp_action::Client<ProcessRequest>::SendGoalOptions();  
  send_goal_options.goal_response_callback = bind(&ActionClient::goal_response_callback, this, placeholders::_1);  
  send_goal_options.feedback_callback = bind(&ActionClient::feedback_callback, this, placeholders::_1, placeholders::_2);  
  send_goal_options.result_callback = bind(&ActionClient::result_callback, this, placeholders::_1);  
  
  // Send the goal to the action server  
  action_client_->async_send_goal(goal_msg, send_goal_options);  
}  

void ActionClient::goal_response_callback(const rclcpp_action::ClientGoalHandle<ProcessRequest>::SharedPtr & goal_handle)  
{  
  lock_guard<mutex> lock(client_mutex_);  
...
void ActionClient::feedback_callback(GoalHandleProcessRequest::SharedPtr goal_handle, const shared_ptr<const ProcessRequest::Feedback> feedback)  
{  
...
void ActionClient::result_callback(const GoalHandleProcessRequest::WrappedResult & result)  
{  
  lock_guard<mutex> lock(client_mutex_);  
  ...
  switch (result.code) {  
    case rclcpp_action::ResultCode::SUCCEEDED:  
      RCLCPP_INFO(this->get_logger(), "Goal succeeded with result: %s", result.result->result_message.c_str());  
      break;  
